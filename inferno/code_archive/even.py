...
def CreateEvenGrid(width, length, prob_iter, tree_density, starting_coordinate_width, starting_coordinate_length):
    trees = tree_density
    grid = generate_grid(width, length, starting_coordinate_width, starting_coordinate_length)
    prob_grid = generate_prob_grid(width, length, starting_coordinate_width, starting_coordinate_length)
        ...
        for j in range(distance):
            ...
            for i in movements:
                for g in all_squares:
                    if (g[0]+i[0] < width and g[0]+i[0] >= 0 and g[1]+i[1] < length and g[1]+i[1] >= 0):

                        if ([g[0]+i[0], g[1]+i[1]] not in all_squares and [g[0]+i[0], g[1]+i[1]] not in all):
